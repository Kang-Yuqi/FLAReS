...
# for counter in reversed(range(len(midpoints_redshift_G))):
# 	print(counter)
# 	redshift = midpoints_redshift_G[counter]
# 	redshift_start = results.redshift_at_comoving_radial_distance(intervals_G[counter][1]/(ql_param['H0']/100))
...
# 	redshift_start_list.append(redshift_start)
...
# 		redshift_start = results.redshift_at_comoving_radial_distance(intervals[counter][0]/(ql_param['H0']/100))
...
# 		win = (chistar-chi)*chi/chistar
# 		map_sum += map_contrast*win/(1/(1+redshift_end))*(intervals[counter][1]-intervals[counter][0])
...
# np.save(f"{os.path.join(root_local, f'lensing/cl_kappa_reverse.npy')}",cl_kappa_sim)
# np.save(f"{os.path.join(root_local, f'lensing/redshift_start_list_reverse.npy')}",redshift_start_list)
# np.save(f"{os.path.join(root_local, f'lensing/redshift_mid_list_reverse.npy')}",redshift_mid_list)
# np.save(f"{os.path.join(root_local, f'lensing/nG_evolution_reverse.npy')}",nG_array)
# np.save(f"{os.path.join(root_local, f'lensing/cl_kappa_list_reverse.npy')}",cl_kappa_list)
...
# cl_kappa_sim = np.load(f"{os.path.join(root_local, f'lensing/cl_kappa_reverse.npy')}")
...
# plt.savefig(os.path.join(root_local, f'lensing/convergence_cl_reverse.png'))
# plt.savefig(os.path.join(root_local, f'lensing/pdf/convergence_cl_reverse.pdf'))
# plt.close()
...
# cl_kappa_list = np.load(f"{os.path.join(root_local, f'lensing/cl_kappa_list_reverse.npy')}")
# redshift_mid_list = np.load(f"{os.path.join(root_local, f'lensing/redshift_mid_list_reverse.npy')}")

# cmap = cm.get_cmap('rainbow', len(redshift_mid_list))
# norm = LogNorm(vmin=min(redshift_mid_list), vmax=max(redshift_mid_list))

# step = 5
# fig, ax = plt.subplots()
# for i, redshift in enumerate(redshift_mid_list[::step]):
# 	ax.loglog(ls_kappa[ql_param['lmin']:-1], cl_kappa_list[step*i][ql_param['lmin']:-1], color=cmap(norm(redshift)))

# sm = cm.ScalarMappable(cmap=cmap, norm=norm)
# sm.set_array([])
# cbar = plt.colorbar(sm, ax=ax, ticks=redshift_mid_list[::step])
...
# plt.savefig(os.path.join(root_local, f'lensing/convergence_cl_evolution_reverse.png'))
# plt.savefig(os.path.join(root_local, f'lensing/pdf/convergence_cl_evolution_reverse.pdf'))
# plt.close()

#### lensing NG evolution####
nG_array = np.load(f"{os.path.join(root_local, f'lensing/nG_evolution_reverse.npy')}")
redshift_start_list = np.load(f"{os.path.join(root_local, f'lensing/redshift_start_list_reverse.npy')}")
...
mask = redshift_start_list < 30
filtered_redshift = redshift_start_list[mask]
...
for i, ax in enumerate(axs.flat):
	...
	ax.set(title=param_names[i], xlabel=r'$z_{>}$')
plt.tight_layout()
plt.savefig(os.path.join(root_local, f'lensing/nG_evolution_reverse_order_zless200.png'))
# plt.savefig(os.path.join(root_local, f'lensing/pdf/nG_evolution_reverse_order.pdf'))
plt.close()
